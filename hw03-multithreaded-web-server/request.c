...
void requestHandle(int fd, time_stats tm_stats, threads_stats t_stats, server_log log)
{
    ...
    if (strcasecmp(method, "GET") == 0) {
        ...
        if (is_static) {
            ...
            t_stats->stat_req++;  // Increment static counter
            ...
            t_stats->dynm_req++; // Increment dynamic counter
        ...
        t_stats->post_req++; // valid post increment
        gettimeofday(&tm_stats.log_enter, NULL);
        body_len = get_log(log, (char**)&body_content, &tm_stats.log_exit);
    ...
    /*if (strcasecmp(method, "GET") == 0) {
        ...
        gettimeofday(&tm_stats.log_enter, NULL);
        int log_len = append_stats(log_buf, t_stats, tm_stats);
        log_buf[log_len++] = '\n';

        add_to_log(log, log_buf, log_len, NULL);
